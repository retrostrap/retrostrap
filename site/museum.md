...
description: "The Museum of the Early Web: how the web looked from 1995 to 2004, why it looked that way, and which piece of retrostrap inherits each exhibit."
---
...
One curator's note before you enter: every exhibit points at a Toybox widget, a theme, or a demo site you can open today; the [components](/components/) and [themes](/themes/) wings show them live. Everything named here is built and shipping, not a wishlist.
...
- The themes are era samples, not fantasies, each traces to a documented genre (section 7). All ten ship today; `highcontrast` is the one deliberate exception, an accessibility-first skin in period dress.
- The demo sites map to eras: homepage-classic (1996-99), fanpage (1997-2000), smallbiz (1999-2001), app-todo and dashboard (bevel-era software), whats-new (2002-04).
...
**Design DNA →**
- Palette Law: all `--rs-*` color tokens live on the 216 cube plus the 16 named colors, silver `#C0C0C0` included.
- Netscape's exact defaults (`#0000EE` links, `#551A8B` visited) sit just off the cube; `classic` answers them with lawful neighbors (`#0000FF`, `#660099`), picked by era taste rather than arithmetic, and documents the substitution.
...
**Design DNA →**
- The homepage-classic demo is this whole section, playable: banner, counter, guestbook links, webring bar, badge wall.
- `rs-counter`, `rs-award`, `rs-webring-bar`, `rs-badge--new`, `rs-card-profile`, `rs-avatar`, `rs-button88` cover the organ inventory; the guestbook, webring, hit-counter and last-updated widgets animate it.
- `rs-construction`: an under-construction sign as a component + original pixel asset, because no museum of this era can lack one.
- "Der Stammtisch" inherits Gästebuch warmth: greeting-first, low-stakes posting, no metrics.
...
**Design DNA →** the neko widget: a pixel cat that chases the cursor or patrols an element edge. Our sprites are original work (Gif the cat's scrappy cousin), drawn in tribute, ripped from nowhere.
...
**Design DNA →**
- Retrostrap Boards run the classic anatomy: "Retrostrap Talk," "The Off-Topic Lounge," and "Der Stammtisch," with rank titles Lurker/Newbie/Member/Regular/Old-Timer and staff titled Sysop and Webmaster.
...
**Design DNA →**
- Motion Law: only `linear` and `steps()` easing; `rs-blink` at a tidy 1s `steps()` cycle; `rs-marquee` quantized to 30/60/120 px/s (a respectful rounding of IE's defaults); everything honors `prefers-reduced-motion`.
- `rs-marquee` pauses on hover and focus and exposes controls, pause-stop-hide as chrome, not chore.
- Decency Law: no autoplaying audio ever; the jukebox exists precisely so music can be offered without being inflicted.
...
**Design DNA →**
- Ten themes = documented dialects; the six core ones map one-to-one to this table. All ten are on the [themes page](/themes/).
...
- The starfield and crt widgets are theme accessories (`midnight`, `phosphor`); sparkle belongs to `kawaii`.
...
**Design DNA →**
- Asset rule: archives are for study; every shipped sprite is original (the asset pipeline enforces this).
- The webring widget + `rs-webring-bar` target the Neocities-era ring revival as first-class users.
- We link every project above; we cite our elders in public.
