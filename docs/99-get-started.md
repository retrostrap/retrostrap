...
build thin framework wrappers if there is real demand for them.
...
Switching at runtime, for a theme picker, goes through the JavaScript API, which also
...
If you switch themes at runtime, make sure the theme's stylesheet is already on the page
(load it up front, or swap a `<link>`'s `href`). All ten themes side by side, with live
swatches, are on the [themes page](https://retrostrap.dev/themes/).
...
retrostrap.dev keeps a comfy-mode toggle in its header if you want to feel the difference.
...
The same laws ship as data in [guardrails.json](https://retrostrap.dev/guardrails.json)
if you'd rather enforce them in your own tooling.

## Where to next

- [Components](https://retrostrap.dev/components/), all 63 of them, with live specimens and copy-paste source.
- [Themes](https://retrostrap.dev/themes/), all ten of them, side by side.
- [The Museum](https://retrostrap.dev/museum/), why the old web looked like this. Bring a beverage.
- [For robots](https://retrostrap.dev/for-robots/), llms.txt, the manifest, the `retrostrap-mcp` server, and a system prompt for your favorite assistant.
- The full [JavaScript API](06-javascript-api.md) once you want the widget engine, events, and the audit surface.
